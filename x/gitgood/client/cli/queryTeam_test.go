...
func networkWithTeamObjects(t *testing.T, n int) (*network.Network, []*types.Team) {
	...
	for i := 0; i < n; i++ {
		state.TeamList = append(state.TeamList, &types.Team{Creator: "ANY", Id: uint64(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Team)
			}
		})
	}
}
...
func TestListTeam(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.Team[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Team)
	})
}
